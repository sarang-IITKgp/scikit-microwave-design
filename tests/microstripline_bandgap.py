...
lambda_g_line = md.structure.Microstripline(er=epsilon_r,h=h_subs,w=1.1*md.MILLI,l=5*md.MILLI,text_tag='Right-line',omega=omega0).lambda_g

d_period = lambda_g_line/2

msl_period_by_2 = md.structure.Microstripline(er=epsilon_r,h=h_subs,w=w_line,l=d_period/2,text_tag='Right-line',omega=omega)


def define_NW_for_open_stub(w_stub,L_stub,omega):
	msl_stub = md.structure.Microstripline(er=epsilon_r,h=h_subs,w=w_stub,l=L_stub,text_tag='stub',omega=omega)
	ZL_stub = md.OPEN
	return md.network.from_shunt_Y(msl_stub.NW.input_admittance(1/ZL_stub))
...
L_stub = lambda_g_stub/10

NW_stub = define_NW_for_open_stub(w_stub,L_stub,omega)

NW_unit_cell = msl_period_by_2.NW*NW_stub*msl_period_by_2.NW


L_defect = d_period/2

msl_defect = md.structure.Microstripline(er=epsilon_r,h=h_subs,w=0.25*w_line,l=L_defect,text_tag='Right-line',omega=omega)



#NW_filter = msl_Tx1.NW*NW_unit_cell**10*msl_Tx1.NW
N_cells = 5

NW_filter = msl_Tx1.NW*NW_unit_cell**N_cells * msl_defect.NW * NW_unit_cell**N_cells * msl_Tx1.NW
